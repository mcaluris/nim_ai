...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        self.q[(tuple(state), action)] = old_q + self.get_alpha(state,
                                                                action) * (reward + future_rewards - old_q)
    ...
    def best_future_reward(self, state):
        ...
        # for action in available_actions:
        #   state[action[0]] = state[action[0]] - action[1]
        #   best_reward = max(self.mini_max(state, truth=False), best_reward)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        available_actions = Nim.available_actions(state)
        best_action = random.sample(available_actions, 1)[0]
        best_reward = -math.inf

        if epsilon and np.random.uniform() < epsilon:
    ...
    def get_gamma(self, state=None, Action=None):
        pass
